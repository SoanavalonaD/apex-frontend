...
import { useRentals } from '../api/rentals/hooks/useRentals';
import { pickImage } from '../api/cars/cars.service';
...
export default function HistoriqueReservations({ setCurrentPage, token }: HistoriqueReservationsProps) {
  const { rentals, loading } = useRentals(token);
  ...
  const formattedReservations = rentals.map(r => {
    const brand = r.car?.brand || 'Apex';
    const type = r.car?.type || 'Luxury';
    return {
      id: r.id,
      vehicleTitle: r.car ? `${r.car.brand} ${r.car.model}` : 'Véhicule Premium',
      vehicleImage: pickImage(brand, type),
      status: r.status,
      dates: `${formatDate(r.start_date)} — ${formatDate(r.end_date)}`,
      totalAmount: parseFloat(r.total_price).toLocaleString(),
      pickup: r.pickup_location || 'Non spécifié',
      dropoff: r.dropoff_location || 'Non spécifié'
    };
  });
  return (
    <div className="pt-24 pb-32 px-container-margin max-w-7xl mx-auto space-y-section-gap">
      ...
      <div className="glass-panel p-8 rounded-xl">
        {!loading && formattedReservations.length === 0 ? (
          /* Empty State */
